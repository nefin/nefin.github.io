---
title: Short Interest
type: datasets
summary: Statistics from Brazilian stock loan market.
last_observation: "2026-07-03"
coverage_start: "2012-11-07"
frequency: "Daily"
description: "Nefin computes a set of statistics for the Brazilian stock loan
  market. These include the short interest (the ratio of stocks on loan to
  stocks outstanding), the days-to-cover (the ratio of stocks on loan to the
  30-day average trading volume) and the loan fee. "
downloads:
  - label: Average Short Interest (CSV)
    url: /resources/stock_loans/average_short_interest.csv
  - label: Average Days-to-Cover (CSV)
    url: /resources/stock_loans/average_days_to_cover.csv
  - label: Average Loan Fee (CSV)
    url: /resources/stock_loans/average_loan_fee.csv
  - label: Methodology (PDF)
    url: /resources/NEFIN_methodology.pdf
---
